...
void QCrioModulesDataExtractor::extractVoltageModules(const QStringList &iniFiles, QComboBox *modulesComboBox, QComboBox *channelsComboBox)
{
    if (!modulesComboBox || !channelsComboBox)
    {
        return; // Safety check to ensure pointers are valid
    ...
    {
        QFileInfo fileInfo(file);
        if (fileInfo.fileName().contains("NI9239", Qt::CaseInsensitive))
        {
            QSettings settings(file, QSettings::IniFormat);
